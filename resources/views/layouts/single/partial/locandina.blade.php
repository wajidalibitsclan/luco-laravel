                            ...
                            <span>
                                @foreach($row['persone'] as $key => $persona)
                                    ...
                                    @if(is_object($termine_persona))
                                        <a href="{{route('termList', ['vocabulary' => 'persone', 'slug' => $termine_persona->slug ])}}">{{trim($termine_persona->termine)}}</a>{{$persona == end($row['persone']) ? '' : ','}}
                                    @else
                    ...
                    @foreach($value['personaggio-interprete'] as $row)
                        <div class="figura">
                            ...
                            @if(isset($row['persone']) && !empty($row['persone']))
                                    ...
                                    @endphp
                                    @if(is_object($termine_persona))
                                        - <span><a href="{{route('termList', ['vocabulary' => 'persone', 'slug' => $termine_persona->slug ])}}">{{$termine_persona->termine}}</a></span>
                                    @else
